...
export default function LoginPage() {
  ...
  async function submit(event) { event.preventDefault(); setError(''); try { const { data } = await api.post('/auth/login', form); navigate('/verify-otp', { state: { challengeId: data.challengeId, demoOtp: data.demoOtp || null } }); } catch (e) { setError(e.response?.data?.message || 'Unable to sign in.'); } }
